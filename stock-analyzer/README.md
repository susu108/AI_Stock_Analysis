...
|--------|---------------------------|------------|
| 股票分析-资讯哨兵-早/晚/夜 | `0 9,18,22 * * *`（每天 09:00 / 18:00 / 22:00，含周末） | `stock-news-watch` |
| 股票分析-资讯哨兵-午 | `30 12 * * *`（每天 12:30，含周末） | `stock-news-watch` |

> cron 一条表达式难以同时表达「整点 + 12:30」，故建议建 **2 个 Job**，`event_type` 均为 `stock-news-watch`。
...
窗口：每天 **09:00 / 12:30 / 18:00 / 22:00** 四定点 ±10 分钟（含周末）。同监控组同标题 **24 小时内**去重。

> 若仍使用旧版 `*/15` 或工作日 `15 9,11,13 * * 1-5`，请到 [cron-job.org](https://console.cron-job.org/) 把「股票分析-资讯哨兵」改成上表 **2 个 Job**，否则触发时刻与代码窗口不一致。
