...
| group | channel | group_label | 钉钉群 |
|-------|---------|-------------|--------|
| `pharma` | `default` | （空） | `DINGTALK_WEBHOOK`（医药/有色共用群） |
| `nonferrous` | `default` | `有色周期` | 同上（标题前缀区分） |
| `oil_short` | `oil` | `石油板块短线` | `DINGTALK_WEBHOOK_OIL` |

```env
DINGTALK_WEBHOOK_OIL=https://oapi.dingtalk.com/robot/send?access_token=...
DINGTALK_SECRET_OIL=SEC...
STOCK_PROFILES=[{"code":"301075",...,"group":"pharma","channel":"default"},{"code":"300497",...,"group":"pharma","channel":"default"},{"code":"000933",...,"group":"nonferrous","channel":"default","group_label":"有色周期"},{"code":"600938",...,"group":"oil_short","channel":"oil","group_label":"石油板块短线",...}]
```

- 配置 `STOCK_PROFILES` 后以其为准；未配置时回退为单股 `STOCK_CODE`
- 医药组：`301075` 多瑞生物、`301201` 诚达药业、`300497` 富祥股份（`pharma` / `default`）
- 有色组：`000933` 神火股份、`000807` 云铝股份（`nonferrous` / `default`，标题 `有色周期｜…`）
...
- 仅有色组：`python main.py --now --group nonferrous`
- 单股：`python main.py --now --stock-code 000933`
- GitHub Actions：`STOCK_PROFILES` 变量 + Secrets `DINGTALK_WEBHOOK_OIL` / `DINGTALK_SECRET_OIL`；workflow matrix 并行 `pharma` / `oil_short` / `nonferrous`，cron-job.org 仍按原次数即可
...
| Variable | `STOCK_PROFILES` | 可选，多股监控 JSON（含医药 + 有色 000933/000807 + 石油等） |
| Variable | `DINGTALK_REPORT_MODE` | 可选，`compact`（默认）或 `full` |
| Variable | `WEB_SEARCH_ENABLED` | 建议设 `true` 以补全隔夜板块重磅（需配置 Secret `WEB_SEARCH_API_KEY`） |
| Variable | `SECTOR_PEER_WATCH` | 可选，同行监控名单逗号分隔（默认含迪哲医药/哈药股份等） |
...
在正式 3 次推送之外，可另建 cron 轮询强催化资讯（医药同行授权/连板、石油地缘强讯），命中后即时推 compact 报告；与定时推送**并存**、互不强占 8 分钟槽位。

| 任务名 | Cron 建议（Asia/Shanghai） | event_type |
|--------|---------------------------|------------|
| 股票分析-资讯哨兵-盘中 | `*/15 9-14 * * 1-5`（工作日约每 15 分钟；窗外由代码门控丢弃） | `stock-news-watch` |
| 股票分析-资讯哨兵-晚/夜 | `0 18,22 * * *`（每天 18:00 / 22:00，含周末） | `stock-news-watch` |

> 建 **2 个 Job**，`event_type` 均为 `stock-news-watch`。盘中 Job 在午休也会触发，但 11:30–13:00 / 15:00 后会被窗口门控跳过。

**Request body：**

```json
{
  "event_type": "stock-news-watch",
  "client_payload": {}
}
```

Headers 与 4.2 相同。工作流：`Stock News Watch`（matrix：`pharma` / `oil_short` / `nonferrous`）。

本地调试：

```bash
python main.py --news-watch --group pharma --ignore-watch-window
```

窗口：工作日盘中 **09:30–11:30 / 13:00–15:00**（配合 cron 约每 15 分钟）；每天 **18:00 / 22:00** ±10 分钟（含周末）。同监控组同标题 **24 小时内**去重；无有效新增不推。

> 若仍使用旧版四定点 `0 9,18,22` / `30 12`，请到 [cron-job.org](https://console.cron-job.org/) 改成上表 **2 个 Job**，否则盘中不会每 15 分钟扫。

建议同时将 GitHub Variable `WEB_SEARCH_ENABLED=true`，以便隔夜授权类新闻进入联网补采。
...
python main.py --news-watch --group oil_short --ignore-watch-window
```
