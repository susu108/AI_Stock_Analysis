...
| `oil_short` | `oil` | `石油板块短线` | 已停用（matrix 已移除；代码保留，恢复时加回 profiles + matrix） |
...
- GitHub Actions：`STOCK_PROFILES` 变量；workflow matrix 并行 `pharma` / `nonferrous`（石油组已停用）
...
Headers 与 4.2 相同。工作流：`Stock News Watch`（matrix：`pharma` / `nonferrous`）。
