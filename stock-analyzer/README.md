...
|--------|---------------------------|------------|
| 股票分析-资讯哨兵 | `15 9,11,13 * * 1-5`（约每 2 小时：09:15 / 11:15 / 13:15） | `stock-news-watch` |
...
窗口：交易日约 `09:05–11:25`、`13:00–14:55`（避开 09:00/11:30/14:30 正式点）。同监控组同标题 **24 小时内**去重。

> 若仍使用旧版 `*/15 ...`，请到 [cron-job.org](https://console.cron-job.org/) 把「股票分析-资讯哨兵」的 Schedule 改成上表表达式，否则仍会约每 15 分钟触发。
