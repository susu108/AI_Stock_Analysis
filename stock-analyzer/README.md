...
STOCK_PROFILES=[{"code":"301075",...,"group":"pharma","channel":"default"},{"code":"300497",...,"group":"pharma","channel":"default"},{"code":"600938",...,"group":"oil_short","channel":"oil","group_label":"石油板块短线",...}]
```

- 配置 `STOCK_PROFILES` 后以其为准；未配置时回退为单股 `STOCK_CODE`
- 医药组示例含 `301075` 多瑞生物、`301201` 诚达药业、`300497` 富祥股份（同 `pharma` / `default` 钉钉群，各推一条）
...
- 单股：`python main.py --now --stock-code 300497`
...
| Variable | `STOCK_PROFILES` | 可选，多股监控 JSON（含 301075 + 301201 + 300497 等） |
...
Headers 与 4.2 相同。工作流：`Stock News Watch`（matrix：`pharma` / `oil_short`）。
